// Shared header and footer, stamped into static HTML between shell markers.
...
// A link renders only when its target exists, so unfinished routes never produce dead links.
export function makeExists(dir = root) {
  return (key) => {
    const target = routes.pages[key];
    if (!target) return true;
    const [file, hash] = target.split('#');
    const p = path.join(dir, file);
    if (!fs.existsSync(p)) return false;
    if (hash) return new RegExp(String.raw`\sid="${hash}"`).test(fs.readFileSync(p, 'utf8'));
    return true;
  };
}
...
export function renderHeader(active, exists = makeExists()) {
  const link = ([key, label]) => exists(key)
    ? `<a href="${href(key)}"${key === active ? ' aria-current="page"' : ''}>${label}</a>`
    : '';
  const items = NAV.map(link).filter(Boolean);
  const book = routes.external.book;
  return `<!-- shell:header -->
<a class="skip-link" href="#main">Skip to content</a>
<header class="site-header">
  <div class="wrap site-header__bar">
    <a class="brand" href="${href('home')}" aria-label="Panoramica Solutions, home">${mark('brand__mark', 'beam-head')}${word('brand__word')}</a>
    <nav class="nav" aria-label="Primary">
      ${items.join('\n      ')}
    </nav>
    <div class="site-header__actions">
      <a class="btn btn--primary btn--sm header-cta" href="${book}" target="_blank" rel="noopener">Book Audit${NEW_TAB}</a>
      <button class="nav-toggle" type="button" aria-expanded="false" aria-controls="nav-panel" aria-label="Open menu">
        <svg class="icon-open" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true"><path d="M4 7h16M4 12h16M4 17h16"/></svg>
  ...
  <nav class="nav-panel" id="nav-panel" aria-label="Mobile" hidden>
    <div class="wrap">
      ${items.join('\n      ')}
      <a class="btn btn--primary" href="${book}" target="_blank" rel="noopener">Book Audit${NEW_TAB}</a>
    </div>
  </nav>
</header>
<!-- /shell:header -->`;
...
export function renderFooter(exists = makeExists()) {
  ...
  company.push(`<li><a href="${routes.external.email}">Email us</a></li>`);
  return `<!-- shell:footer -->
<footer class="site-footer">
  <div class="wrap">
    <a class="footer-brand" href="${href('home')}" aria-label="Panoramica Solutions, home">
      <span class="footer-brand__row">${mark('footer-brand__mark', 'beam-foot')}${word('footer-brand__word')}</span>
      <span class="footer-brand__sub" aria-hidden="true">Solutions</span>
    </a>
  </div>
  <div class="wrap footer-grid">
    ...
    <nav aria-label="Legal">
      ...
      <ul class="footer-list">
          ...
          <li><a href="${href('terms')}">Terms and Conditions</a></li>
      </ul>
    </nav>
  </div>
  <div class="wrap footer-base"><p>&copy; 2026 Panoramica Solutions. All rights reserved.</p></div>
</footer>
<!-- /shell:footer -->`;
}

const block = (name) => new RegExp(String.raw`<!-- shell:${name} -->[\s\S]*?<!-- /shell:${name} -->`);

export function hasShell(html) {
  return block('header').test(html) && block('footer').test(html);
}

export function applyShell(html, exists = makeExists()) {
  const m = html.match(/<body[^>]*\sdata-nav="([^"]*)"/);
  const active = m ? m[1] : '';
  return html
    .replace(block('header'), () => renderHeader(active, exists))
    .replace(block('footer'), () => renderFooter(exists));
}
