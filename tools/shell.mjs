...
<header class="site-header">
  <div class="wrap site-header__bar">
    <a class="brand" href="${href('home')}" aria-label="Panoramica, home">${LOGO}</a>
    <nav class="nav" aria-label="Primary">
      ${items.join('\n      ')}
    </nav>
    <div class="site-header__actions">
      <a class="btn btn--primary btn--sm header-cta" href="${book}" target="_blank" rel="noopener">Book a call${NEW_TAB}</a>
  ...
  <nav class="nav-panel" id="nav-panel" aria-label="Mobile" hidden>
    <div class="wrap">
      ${items.join('\n      ')}
      <a class="btn btn--primary" href="${book}" target="_blank" rel="noopener">Book a call${NEW_TAB}</a>
    </div>
  </nav>
</header>
<!-- /shell:header -->`;
...
export function renderFooter(exists = makeExists()) {
  ...
  const offers = OFFERS.map(li).filter(Boolean).join('\n          ');
  const company = [['diagnostic', 'Free diagnostic'], ['about', 'About'], ['insights', 'Insights']].map(li).filter(Boolean);
  company.push(`<li><a href="${routes.external.book}" target="_blank" rel="noopener">Book a call${NEW_TAB}</a></li>`);
...
<footer class="site-footer">
  <div class="wrap footer-grid">
    ...
    <nav aria-label="What we fix">
      <h2 class="footer-h">What we fix</h2>
